...
/// <summary>
/// Chat service. This is a deliberately thin HTTP client of the Forager Server's
...
public class BaseChatService
{
   ...
   /// the server and its answer returned. Any failure along the way is swallowed and reported
   /// as a friendly "server unreachable" message rather than surfacing a raw exception to the
   /// user.
   /// </summary>
   ...
   public virtual async Task<string> AskQuestionAsync( string question )
   {
      try
      {
         ...
         return await PostChatAsync( question );
      }
      catch( Exception )
      {
   ...
   /// <summary>
   /// Starts a fresh session from the user's point of view. The server is stateless and the
   /// client keeps no conversation transcript, so there is nothing to clear here; the method
   /// exists as a hook the UI can call when the user resets the chat.
   /// </summary>
