...
namespace AzureDevOpsForager.Core.Services.Chat
{
   ...
   /// The provider is effectively stateless: every request carries its own system prompt,
   /// conversation history, and code context, so nothing needs to be retained between calls.
   /// </summary>
   public class GroqProvider : ILLMProvider
   {
      ...
      /// ordered messages array (system prompt, then recent conversation history, then the
      /// current question with any code context appended), posted to Groq, and the model's reply
      /// is extracted from the JSON response. Any failure, whether an HTTP error or a thrown
      /// exception, is returned as an inline error string rather than propagated, because the
      /// chat UI displays whatever text comes back.
      /// </summary>
      ...
      /// <param name="conversationHistory">Prior turns, used to keep the model in context.</param>
      /// <returns>The model's answer, or an error string prefixed with the failure marker.</returns>
      public async Task<string> AskAsync( string question, string context, List<object> conversationHistory )
      {
         try
         {
            var messages = BuildMessages( question, context, conversationHistory );
      ...
      /// <summary>
      /// No-op reset. Groq is a stateless HTTP API, so there is no server- or client-side
      /// conversation state to clear; the method exists only to satisfy the interface.
      /// </summary>
      public void ResetConversation()
      {
         // Nothing to reset: every request already carries its own history and context.
      }

      /// <summary>
      /// Human-readable provider name used for logging and display in the chat UI.
      /// </summary>
      public string ProviderName => "Groq (llama-3.3-70b)";
      ...
      /// most recent slice of conversation history, then the current user turn. Splitting this
      /// out keeps <see cref="AskAsync"/> focused on the request/response round trip.
      /// </summary>
      private List<object> BuildMessages( string question, string context, List<object> conversationHistory )
      {
      ...
      /// <summary>
      /// Appends the tail of the conversation history to the outgoing messages. Only the last
      /// ten turns are included so the request stays comfortably within the model's token budget
      /// while still giving it enough recent context to stay coherent.
      /// </summary>
      private void AppendRecentHistory( List<object> messages, List<object> conversationHistory )
      {
